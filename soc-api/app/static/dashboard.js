...
function renderEvents(events) {
  ...
  qs("eventsCount").textContent = `${events?.length ?? 0} shown`;
...
function computeStatsFromPayload(payload) {
  ...
  return { window: "from page payload", events_per_min, alerts_by_severity, top_src_ips };
}
...
function ensureCharts(payload) {
  ...
  if (c1) {
    ...
    const labels = stats.events_per_min.map(x => x.minute.slice(11));
